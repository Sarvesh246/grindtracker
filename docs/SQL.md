...
| [14-rest-days.sql](sql/14-rest-days.sql) | 14 | **Rest days.** A streak used to require consecutive calendar dates, which no real training program produces. Adds `user_rest_settings` (weekly rest weekdays — part of your program, so unlimited) and `user_rest_dates` (ad-hoc rest passes, **2 per rolling 7 days**, claimable only for the last 7 days). Passes go exclusively through the `claim_rest_days` RPC: the rationing can't be expressed in a policy without the policy subquerying its own table, and obviously can't be left to the client, so client INSERT/UPDATE/DELETE on `user_rest_dates` is revoked. `grind_recompute_stats()` is rewritten so a new streak run starts only when the gap back to the previous workout contains a NON-rest day. A rest day bridges but never counts, so the streak number stays "workouts in this unbroken run" and the +50 XP milestone now fires every 7th workout of a run rather than every 7th calendar day — reachable for the first time by anyone training fewer than 7 days a week. Nothing is migrated: stats are derived, so turning rest days on (or changing them) re-derives history on the next recompute. **Apply after `13`.** |
