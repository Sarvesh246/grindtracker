...
--
-- Stores coach chat turns so free-tier Gemini usage can be rate-limited per user.
-- RLS: users insert/select only their own rows. Rate limits are enforced by a
-- BEFORE INSERT trigger on user-role rows only (assistant replies are free against
-- the user quota but still stored for optional history).
--
-- Mirror limits in src/lib/coach/constants.ts — change both places together.
...
-- No update/delete policies — transcripts are append-only from the client.

-- ── Rate limit (user role only) ────────────────────────────────────────────
-- Burst: 8 user messages / 10 minutes
-- Daily: 15 user messages / 24 hours
-- Tagged exceptions map to plain English in /api/coach/chat.
