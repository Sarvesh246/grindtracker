...
begin
  ...
  select
    bool_and(c in ('push','pull','legs')) is not null
      and count(distinct case when c in ('push','pull','legs') then c end) = 3,
    bool_or(extract(dow from s.local_date) = 0)
      and bool_or(extract(dow from s.local_date) = 6)
    into v_has_full_split, v_has_weekend
    from sessions s
    left join user_day_categories udc
      on udc.user_id = v_user and udc.day_key = s.day_type
    cross join lateral (
      select coalesce(udc.category, s.day_type) as c
    ) cat
  ...
  -- Fix full-split properly (the bool_and above is wrong when empty).
  select
    count(distinct case
