...
export default function BodyWeightCard() {
  ...
  const { unit, unitLabel, toDisplay, toStorage } = useUnit()
  ...
  async function load() {
    ...
    if (todayRow) setDraft(String(toDisplay(todayRow.weight)))
    setLoading(false)
  }
  ...
  async function handleSave() {
    ...
    await supabase.from('body_weights').upsert(
      { user_id: user.id, weight: toStorage(w), recorded_at: todayDateKey() },
  ...
  const change = latest && earliest && latest.recorded_at !== earliest.recorded_at
    ? Math.round((toDisplay(latest.weight) - toDisplay(earliest.weight)) * 10) / 10
    : 0
